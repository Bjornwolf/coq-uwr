...
(** Write down an interesting theorem about bags involving the
    ...
    stuck! *)
Theorem inc_count : forall (b : bag) (n : nat),
  count n (add n b) = S (count n b).
Proof.
  intros b n. 
  assert (H: forall x : nat, beq_nat x x = true).
    intros x. induction x as [|x']. reflexivity. simpl. rewrite IHx'. 
    reflexivity.
  simpl.
  induction b as [| h t]. 
...
Proof.
  intros l. induction l as [|h t].
    reflexivity.
    simpl.
    assert (rev_snoc: forall (l : natlist) (n : nat), rev (snoc l n) = n :: rev l).
      intros l n. induction l as [| h' t'].
        reflexivity.
        simpl. rewrite IHt'. reflexivity.
    rewrite rev_snoc. rewrite IHt. reflexivity.
Qed.
...
Proof.
  ...
  induction l1 as [| h t].
    ...
    simpl. rewrite IHt. 
    assert (snoc_app_dist: forall (xs ys : natlist) (n : nat), snoc (xs ++ ys) n = xs ++ snoc ys n).
      intros xs ys n. induction xs as [|x xs'].
        reflexivity.
        simpl. rewrite IHxs'. reflexivity.
    rewrite snoc_app_dist. reflexivity.
Qed.
...
Theorem rev_injective : forall l1 l2 : natlist,
  rev l1 = rev l2 -> l1 = l2.
Proof.
  intros l1 l2. intros H. rewrite <- rev_involutive. rewrite <- H. 
...
Proof.
  intros d m n o. intros H. simpl. rewrite H. reflexivity.
Qed.
(** [] *)
