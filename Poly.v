...
Proof.
  intros X. intros v s. induction s as [| h t].
    simpl. reflexivity.
...
Theorem rev_involutive : forall X : Type, forall l : list X,
  rev (rev l) = l.
Proof.
  intros X. intros l. induction l as [|h t].
    simpl. reflexivity.
...
Proof.
  intros X. intros l1 l2 v. induction l1 as [|h t].
    simpl. reflexivity.
...
Proof.
  intros X Y. intros f l. induction l as [|h t].
    simpl. reflexivity.
    assert(snoc_app: forall (T' : Type) (xs' : list T') (x' : T'), 
           xs' ++ [x'] = snoc xs' x').
      intros T'. intros xs' x'. induction xs' as [|h'' t''].
        simpl. reflexivity.
        simpl. rewrite IHt''. reflexivity.
    simpl. rewrite <- snoc_app. rewrite <- snoc_app.
    rewrite <- IHt.
    assert(map_app: forall (T1 T2 : Type) (f : T1 -> T2) (xs ys : list T1),
           map f (xs ++ ys) = map f xs ++ map f ys).
      intros T1 T2. intros f0 xs ys. induction xs as [|h' t'].
        simpl. reflexivity.
        simpl. rewrite IHt'. reflexivity.
...
Proof.
  intros X Y. intros l f. induction l as [|h t].
    simpl. reflexivity.
    simpl. rewrite IHt. reflexivity.
Qed.
